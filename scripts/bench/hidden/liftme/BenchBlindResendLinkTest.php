...
class BenchBlindResendLinkTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_open_guest_quotes_get_one_email_with_their_links(): void
    {
        ...
        foreach ([$owned, $someoneElse] as $q) {
    ...
    public function test_bad_input_and_flooding_are_refused(): void
    {
        ...
        // Some flood limit, whatever it is: an unauthenticated form that sends email.
        $codes = [];
        for ($i = 0; $i < 30; $i++) {
            $codes[] = $this->postJson('/api/quotes/resend-link', ['email' => 'a@example.com'])->status();
        }
